...
pll inverseOf(int num){
    ...
    for(int i = 2; i * i <= num; i++){
        ...
        count %= k;
        red = red * binexp(i, count);
        if(count == 0) continue;
        count = k - count;
        inv = inv * binexp(i, count);
    }
    if(num > 1) {
        inv = inv * binexp(num, k - 1);
        red = red * num;
    }
    return {inv, red};
}
...
int main()
{
    ...
    for(int i = 0; i < n; i++){
        ...
        //dbg(numbers);
        //dbg(t);
        pll processed = inverseOf(t);
        //dbg(processed);
        //dbg(numbers[processed.second]);
        total += numbers[processed.second];
        numbers[processed.first]++;
        //dbg(total);
    }
    cout << total << endll;
