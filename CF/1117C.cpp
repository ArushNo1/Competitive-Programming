...
int main()
{
    ...
    ll xdist = abs(x2 - x1), ydist = abs(y2 - y1);
    ll dist = xdist + ydist;
    ...
    /*
    is it possible to reach the destination within x days?
    @param days number of days
    */
	auto eval = [&](ll days){
        if(dist > 2 * days || xdist > 2 * days || ydist > 2 * days){
            return false;
        }
        ll windx = 0, windy = 0;
        for(char& w : wind){
        ...
        ll cycles = days / n;
        windx = windx * cycles + x1;
        windy = windy * cycles + y1;
        //dbg(windx);
        //dbg(windy);
        ll days_left = days % n;
        //dbg(days_left);
        ...
        //dbg(windx);
        //dbg(windy);
        ll travel_dist = abs(windx - x2) + abs(windy - y2);
        //dbg(travel_dist);
        return travel_dist <= days;
    };

    ll low = 1, high = INT64_MAX / 2;
    while(low < high){
        ll mid = (low + high) / 2;
        //dbg(mid);
        if(eval(mid)){
