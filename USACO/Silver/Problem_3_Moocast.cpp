...
inline double dist2(const cow& a, const cow& b){
    return (pow(a.x - b.x, 2) + pow(a.y - b.y, 2));
}
ostream& operator<<(ostream& os, const cow& a){
    return os << "(" << a.x << ", " << a.y << ")-" << a.r;
};
...
int main()
{
    ...
    for(int i = 0; i < n; i++){
        double x, y, r;
        cin >> x >> y >> r;
        cows.push_back({x, y, r * r});
        for(int j = 0; j < i; j++){
            double d = dist2(cows[i], cows[j]);
            if(d <= cows[i].r){
                adj[i].push_back(j);
            }
            if(d <= cows[j].r){
