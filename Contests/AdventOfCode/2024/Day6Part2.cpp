...
bool willgetstuck(int x, int y, int direction, vector<string>& grid, vvi& visited){
    visited = vvi (grid.size(), vi(grid[0].size(), 0));
    while(true){
        int z = 100 * x + y;
...
void solve(vector<string>& grid)
{
    ...
    for(int i = 0; i < grid.size(); i++){
        for(int j = 0; j < grid[i].size(); j++){
            if(grid[i][j] == '.'){
                grid[i][j] = '#';
                bool q = willgetstuck(x, y, direction, grid, visited);
                total += q;
                if(total == 1 && q){
                    dbg(visited);
                }
                //cerr << __LINE__ << ": " << i << " " << j << " " << q << endll;
                grid[i][j] = '.';
            }
        }
    }
