...
double x, y;
#define conv 180.0/3.141592653589793238463

ld calc(ld a, ld b){
	return (conv) * (atan2(b - y, a - x));
}

ld anglesub(ld l, ld r)
{
    return r - l;
}
...
void solve(int num_tc)
{
	...
	cin >> x >> y;
	for(size_t i = 0; i < n; i++){
		angles[i] = calc(positions[i].first, positions[i].second);
    ...
    for(int r = 0; r < angles.size(); r++){
        while(anglesub(angles[l], angles[r]) > theta && l < r) 
        {
            l++;
        }

        //dbg(anglesub(angles[l], angles[r])) dbg(angles[l]) dbg(angles[r])
