...
struct file{
    ...
    friend ostream& operator<< (ostream& os, const file& f){
        return os << "[" << f.start << "+" << f.length << ": " << f.id << "]";
    }
};
...
void solve(string s)
{
    ...
    for(int i = 0; i < s.size(); i++){
        if(i % 2 == 0){
            ...
            // }
            blocks.push_back({position, s[i] - '0', fileid, false});
            fileid++;
        }
        else{
            // for(int j = 0; j < s[i] - '0'; j++){
            //     disk.push_back(-1);
            // }
            blocks.push_back({position, s[i] - '0', -1, true});
        }
        position += s[i] - '0';
    }
    ...
    for(int i = blocks.size() - 1; i >= 0; i--){
        ...
        if(!blocks[i].empty){
            for(int j = 0; j < blocks.size(); j++){
                if(blocks[j].empty && blocks[j].length >= blocks[i].length){
                    cerr << "switching with" << blocks[j] << endll;
                    blocks[i].start = blocks[j].start;
                    blocks[j].start = blocks[i].start + blocks[i].length;
