...
template <typename T, typename U>
ostream& operator<< (ostream& os, const multiset<T, U>& arr){
    os << "[";
    for(const T x : arr){
        os << x << " ";
    }
...
void solve(int num_tc)
{
    ...
    vi subseqs;
    for(int i = 0; i < n; i++){
        int index = lower_bound(all(subseqs), nums[i]) - subseqs.begin();
        if(index >= subseqs.size()){
            subseqs.push_back(nums[i]);
        }
        else{
            subseqs[index] = nums[i];
        }
    }
    
    cout << subseqs.size() << endll;
}
